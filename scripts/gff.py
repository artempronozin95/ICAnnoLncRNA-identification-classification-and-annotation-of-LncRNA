...
df.to_csv('data/output/statistic_bed.tsv', sep='\t', index=False, header=None)

f, ax = plt.subplots(figsize=(15, 5))
ax.bar(df[6], df['number'], color='b')
plt.ylabel('Number of aligned lnRNAs', size=15)
plt.xlabel('Exon of the gene', size=15)
plt.xticks(range(max(df[6])+1))
