...
if config['expression']['option'] == 'on':
    myoutput = ["data/output/expression/table_type.txt", "data/output/expression/entropy.png"] 
if config['expression']['option'] == 'off':
    myoutput = []
...
rule all:
    input:
        ...
        "data/output/GMAP/statistic_ref.csv",
        ...
        "data/output/lncRNA_structure/anti_vs_mrna.tsv",
...
rule filter_long_introns:
    ...
    output:
       "data/output/GMAP/filter_alignm.bed",
       "data/output/lncRNA_structure/itron_coordin.tsv",
       "data/output/lncRNA_structure/itron_small.txt"
    shell:
...
rule statistic:
    input:
        ...
        "data/output/lncRNA_structure/itron_small.txt"
    params:
        "data/output/lncRNA_structure/lncrna_statistic.txt"
    output:
        "data/output/lncRNA_structure/anti_vs_mrna.tsv"
    shell:
...
rule tissue:
    ...
    params:
       ...
       old_new="data/input/new_vs_old_id.tsv",
       SRX=expand("{SRX}", SRX=config['tissue']['SRX_exp'])
    output:
       "data/output/tissue/tissue_org.png"
    run:
...
if config['expression']['option'] == 'on':
  rule build_expr_table:
    input:
       control="data/output/tissue/tissue_org.png",
       SRX=expand("{SRX}", SRX=config['tissue']['SRX_exp'])
    params:
       expand("{path}", path=config['expression']['path']),
       expand("{calculation_type}", calculation_type=config['expression']['calculation_type']),
       "data/input/new_vs_old_id.tsv",
       "data/output/tissue/cons.txt",
       "data/output/tissue/non.txt",
       "data/output/tissue/cod.txt",
       "data/output/expression/expr_table.tsv"
    output:
       "data/output/expression/table_type.txt"
    shell:
       "python scripts/expr_table_form.py {input.SRX} {params} {output}"

  rule entropy:
    input:
       control="data/output/expression/table_type.txt"
    params:
       table=ancient("data/output/expression/expr_table.tsv")
    output:
       "data/output/expression/entropy.png"
    shell:
